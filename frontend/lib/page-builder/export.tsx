import type { PageSchema, ElementNode, ElementStyles } from "./types"
...
function generateClassName(styles: ElementStyles, additionalClasses: string = ""): string {
...
function elementToJSX(element: ElementNode, indent: number = 2): string {
  ...
  switch (element.type) {
    ...
    case "section":
      ...
      const childrenJSX = element.children.map((child) => elementToJSX(child, indent + 2)).join("\n")
      return `${spaces}<section${sectionClass}${styleProp}>\n${childrenJSX}\n${spaces}</section>`
    default:
      return ""
  }
...
export function exportNextJS(schema: PageSchema): string {
  const elementsJSX = schema.elements.map((el) => elementToJSX(el, 4)).join("\n")
