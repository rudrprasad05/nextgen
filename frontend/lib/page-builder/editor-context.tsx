...
import type { PageSchema, ElementNode, ElementType, ElementStyles } from "./types"
import { DEFAULT_PROPS, DEFAULT_STYLES } from "./types"
...
interface EditorContextType {
  ...
  updateElement: (id: string, updates: Partial<Pick<ElementNode, "props" | "styles">>) => void
  deleteElement: (id: string) => void
  moveElement: (activeId: string, overId: string, parentId?: string | null) => void
...
function deleteElementFromTree(elements: ElementNode[], id: string): ElementNode[] {
  return elements
    .filter((el) => el.id !== id)
    .map((el) => {
      if (el.children) {
...
function removeElementFromTree(elements: ElementNode[], id: string): { elements: ElementNode[]; removed: ElementNode | null } {
...
function insertElementAtIndex(
  ...
  return elements.map((el) => {
    if (el.id === parentId && el.children) {
      const targetIndex = el.children.findIndex((child) => child.id === targetId)
      if (targetIndex === -1) {
    ...
    if (el.children) {
      return { ...el, children: insertElementAtIndex(el.children, element, targetId, parentId) }
    }
    return el
  })
}

function addElementToParent(elements: ElementNode[], element: ElementNode, parentId: string): ElementNode[] {
  return elements.map((el) => {
    if (el.id === parentId) {
      return { ...el, children: [...(el.children || []), element] }
    }
...
export function EditorProvider({ children }: { children: ReactNode }) {
  ...
  const addElement = useCallback((type: ElementType, parentId?: string | null) => {
    ...
    setSchema((prev) => {
      if (parentId) {
        return { elements: addElementToParent(prev.elements, newElement, parentId) }
      }
  ...
  const updateElement = useCallback((id: string, updates: Partial<Pick<ElementNode, "props" | "styles">>) => {
    setSchema((prev) => ({
  ...
  const moveElement = useCallback((activeId: string, overId: string, parentId?: string | null) => {
    setSchema((prev) => {
      const { elements, removed } = removeElementFromTree(prev.elements, activeId)
      if (!removed) return prev
      
      if (parentId === undefined) {
        return { elements: insertElementAtIndex(elements, removed, overId, null) }
      }
      
      return { elements: insertElementAtIndex(elements, removed, overId, parentId) }
    })
  }, [])

  const getElement = useCallback((id: string): ElementNode | null => {
